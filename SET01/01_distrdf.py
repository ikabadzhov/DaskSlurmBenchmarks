...
cluster = SLURMCluster(memory='{}g'.format(16*args.cores),
                       processes=args.cores,
                       cores=args.cores,
                       queue='photon',
                       #python='python',
                       local_directory='/tmp/ikabadzh',
                       header_skip=['-n 1', '-t 00:30:00'],
                       job_extra=['--ntasks-per-node=1', '-t 8:00:00'])
...
if __name__ == '__main__':
    ...
    filelist = ["input_files/dimuon/f{}.root".format(i) for i in range(1,1001)]
    df = RDataFrame("Events", filelist, npartitions=(8192), daskclient=client)
